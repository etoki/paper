...
**Manuscript draft**: 2026-05-09 (numbers traceable to `papers/P3_meta_analysis/iceri/results/`)
...
The primary-pool dataset is `papers/P3_meta_analysis/inputs/studies.csv`, derived from the parent preprint extraction. We retain studies with `inclusion_status in {include, include_with_caveat, include_COI}` and `primary_achievement in {yes, partial}` and at least one extractable Pearson r per trait.

### 3.2. Education-level collapsing

Raw education-level codes were collapsed to four categories using the mapping in `papers/P3_meta_analysis/iceri/scripts/run_cross_tab_meta.py::LEVEL_MAP`:
...
Disciplines are derived in `papers/P3_meta_analysis/inputs/derive_studies_csv.py::classify_discipline`:
...
All numerical results are produced by `papers/P3_meta_analysis/iceri/scripts/run_cross_tab_meta.py`. Pooling primitives (Fisher z, REML, HKSJ-adjusted CI) are re-used from `metaanalysis/analysis/pool.py`.
...
- Analysis script: `papers/P3_meta_analysis/iceri/scripts/run_cross_tab_meta.py`.
- Result CSVs / MD: `papers/P3_meta_analysis/iceri/results/{cross_tab_pools, interaction_terms, cross_tab_summary}`.
