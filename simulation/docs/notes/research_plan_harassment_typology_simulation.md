...
- **RQ2.3**：Structural 介入（個人 personality 不変、組織 / 制度のみ変更）の効果は？（Counterfactual C、Pruckner 2013 + Bezrukova 2016 + Dobbin & Kalev 2018 で **20% 削減を main、10–30% range の保守的設定**）
- **RQ2.4**：MHLW 2016 → 2024 の 12.1pp 減少は **policy-coincident decline（準自然実験的 evidence）** として、本研究 Counterfactual C 予測値とどの程度整合するか？（厳密な causal estimate ではなく plausibility 補強として参照、Part 1.4 注釈参照）
...
#### ⚠ 限界 11：Construct Validity（測定が "latent propensity" を本当に測っているか）★ 追加
- **問題**：本研究の core 仮定 — 「N=354 harassment scale (Tou 2017 power harassment + Kobayashi 2010 gender harassment) は **latent harassment propensity** を測定している」 — は **経験的に未検証**。これらの scale の items は (a) "過去に〜をした" の **past behavior reporting**、(b) "状況によっては〜しうる" の **trait-style propensity** のどちらに近いかが、研究設計上は混在している可能性
- **対応**：
  - **Item content review**：Tou 2017 + Kobayashi 2010 の各 item を "behavior reporting" vs "propensity rating" に分類し、両カテゴリの subscore を分離して analyze（supplementary）
  - **Convergent validity check**：本研究の cell-conditional probability が、既存 meta-analysis の HEXACO HH → workplace deviance association（Pletzer 2019、ρ ≈ -0.20 to -0.35）と integration 整合するか確認
  - **Discriminant validity check**：harassment scale が depression / job stress / general negative affect と **過度に相関しない** ことを確認（過度に相関すれば、measure が distress-proxy で latent propensity を測っていない）
...
ここで effect_C ≈ 0.20（main、保守側再校正）、sensitivity range 0.10–0.30（30% は上限、Tier 2 4 系統 triangulation で確定。50% は domain gap を考慮し overoptimistic として除外）
```
...
- → **30% 削減は range 上限（main は 20%）、効果は intensive margin に限定可能性**
...
## Part 7：倫理的配慮（Ethical Considerations）

> **本 Part の目的**：harassment 研究は被害者・加害者・社会の三方向に sensitive な topic である。本研究の publication / dissemination が二次的危害を生まないよう、authorial framing、誤用防止、被害者保護の 3 軸で対応指針を明示する。

### 7.1 Sensitive Topic としての harassment 研究の特性

本研究は以下の点で **特別な ethical attention** を要する：

1. **被害者の re-traumatization リスク**：national-scale prevalence（"被害者数 X 万人"）の publication が、当該経験を持つ読者に "I am one of those statistics" の自己同定を喚起しうる
2. **加害者の stigmatization リスク**：personality cluster と加害確率の関連付けが、特定 personality profile の人々の **集合的 stigma** に転用されうる
3. **個人スクリーニング誤用リスク**：「Cluster 0 = high-risk type」が雇用・昇進・採用での **個人プロファイリング差別** に転用されうる
4. **政策提言の誤読リスク**：「targeted intervention（B）が最も効果的」が、当事者の自由意思を無視した強制カウンセリング・強制研修に転用されうる
5. **加害行為の normalization リスク**：「personality が原因」「変えられる」という framing が、加害者の **行為への責任** を弱める方向に誤読されうる

### 7.2 Authorial Framing Guidelines（執筆時の方針）

論文 / 著作 / プレスリリース等の発信物すべてで以下を遵守：
...
| "high-risk individuals" | "cell with elevated baseline rate (population-level statistical pattern)" |
| "Cluster 0 should be screened" | "Cluster 0 represents a target group for **opt-in voluntary intervention**" |
| "victims of personality types" | "individuals affected by harassment in workplace contexts" |
| "predict perpetrators" | "predict population-level prevalence" |
| "effective intervention reduces harassment" | "anchor literature suggests intervention can reduce harassment under specified conditions; transportability requires further validation" |

#### 7.2.2 Victim-centric framing

- Discussion で被害者を "受動的 statistic" として扱わず、**被害が組織的・社会的責任** であることを明示
- 被害者支援リソース（厚労省パワハラ相談窓口、各都道府県労働局等）を Discussion / 著作末尾に **明示的に列挙**
- "もし本論文を読んで自身の経験が想起された方へ" の reader-care note を著作・プレスリリースに添える（学術論文では footnote 化）

#### 7.2.3 Perpetrator-related framing

- Cluster X の人を **"加害者である" と決めつけない**：「Cluster X profile を持つ多数の人は加害行為をしない；統計的に prevalence が高めなだけ」を繰り返し明示
- 加害行為の **行為責任は変わらない**：「介入可能 = 自己責任から解放」という short-cut を avoid
  - 推奨表現："Personality-based intervention represents one **upstream lever** for population-level reduction; this does not absolve individual responsibility for harmful acts."
- Capability approach の核心：「変化の機会を提供する社会の責任」と「行為に対する個人の責任」は **両立する**

### 7.3 個人スクリーニング誤用防止（Anti-screening Statement）

論文 Methods / Discussion / 著作の各所に以下相当の **明示的 disclaimer** を配置：
...
加えて以下を明示：
- **Cluster 0 でも加害確率は < 100%**（統計的傾向であり個人の決定論ではない）
- **Cluster 1–6 でも加害は発生する**（介入対象を Cluster 0 のみに限定すると残余 risk を見逃す）
- **personality は変化可能**（Roberts 2017 systematic review）→ Cluster は個人の固定属性ではない
- **採用・昇進判断への利用は研究倫理違反**（IRB 申請時に明記）

### 7.4 政策提言の誤用防止（Misuse-prevention Framing）

#### 7.4.1 "Voluntary, opt-in" の堅持

Counterfactual B（targeted intervention）の政策提言は以下条件を必須とする：
- **Voluntary participation**：強制的な研修・カウンセリング受講ではなく、本人意思に基づく opt-in
- **No employment consequence**：参加の有無が雇用条件・評価に影響しない
- **Anonymity / Confidentiality**：参加者の personality profile が組織内で同定可能な形で扱われない
- **Resource provision, not coercion**：社会の役割は「変化を望む個人に支援を提供する」こと、「変化を強制する」ことではない

#### 7.4.2 Capability approach との整合

Doc 2 Part 3 の capability approach に従い：
- **区別 (distinction) は許容**：「Cluster 0 への enhanced support」は capability 拡張のための区別
- **差別 (discrimination) は禁止**：「Cluster 0 の人を排除・降格」は capability 剥奪
- 両者の境界線：**当事者の利益になるか / 不利益になるか**

### 7.5 被害者保護（Victim Protection）

#### 7.5.1 データ取扱い

- N=354 / N=13,668 ともに **既存 IRB 通過済データ**（新規収集なし）
- 公開時：個人特定可能な情報は完全削除済み（既存論文の公開方針に準拠）
- 統計的再同定 risk：cell N が小さい（特に 28-cell の N<10）ため、cell 単位の raw data 公開は **慎重判断**（aggregated statistics のみ公開、raw は restricted access）

#### 7.5.2 Communication strategy（一般向け発信）

- **メディア向け press release で避ける表現**：「○○タイプは加害者になりやすい」のような sensational headline
- **推奨 framing**：「全人口の harassment prevalence を予測する集団モデル。個人のスクリーニングではなく、集団介入の設計に資する」
- **Q&A 想定**：誤読されやすい質問（"Cluster 0 を雇うべきでないか？"）への準備済み回答を保有

### 7.6 Researcher Reflexivity（研究者の立場性）

#### 7.6.1 Positionality statement（論文に明記）

> "The first author has previously argued in non-peer-reviewed work for a systemic-causation framing of workplace harassment, emphasizing social systems' responsibility over individual self-responsibility (Tokiwa, *forthcoming*). This normative stance may influence which limitations are emphasized in the present manuscript and how findings are framed in the Discussion. To mitigate, (a) the empirical analysis section is restricted to L1 descriptive/predictive claims, (b) all causal language is constrained by the target trial emulation framework (Hernán & Robins, 2020) with explicit identifying assumptions, (c) anti-screening and anti-discrimination statements are included regardless of findings, and (d) limitations are presented in a structured 10-item framework derived from a 60-paper literature audit, not selectively."
...
- 既存データ再利用のため新規 IRB 不要だが、**publication 前に所属機関の研究倫理委員会に notification**（既存データの secondary analysis として）
- Pre-registration（OSF）で **本 Part 7 全項目を ethical commitment として明示**
- 論文投稿先誌（JBE 等）の ethics policy に準拠した COI / consent / data availability statement

### 7.8 Long-term ethical monitoring

論文 publication 後の継続的責任：
- **誤用 case の monitoring**：本研究結果が discrimination / screening に転用された case が報告された場合、対応的に correction / commentary を発表
- **二次解析 collaboration の選別**：本研究データを使った secondary study の協力依頼に対し、本 Part 7 の ethical principle を満たすかを審査
- **Author contact 維持**：論文の corresponding author 連絡先を 10 年以上維持（誤用 inquiry 受付）
...
| D10 | 構造介入の effect_C | **20% 削減 main、0.10–0.30 sweep**（保守側再校正、30% は上限）、Tier 2 4 系統 triangulation | ✓ **確定** |
...
| 戦略 5 Pre-registration | **OSF Registration を Stage 0 実装前に固定**（D12 修正版）、failure result publication commitment（D-NEW8）含む |
| 戦略 6 ツール positioning | "policy planning tool" 明示 |
| 評価ポイント A-D（LLM） | 該当なし（LLM 不使用） |
| 評価ポイント E-L | E, F, G, I, J, K, L すべて満たす予定。H（power analysis）は D13 で完了 |
| **追加：倫理的配慮** | **Part 7 全 8 セクションで対応**（被害者保護、個人スクリーニング誤用防止、政策誤用防止、reflexivity statement 等）|
| **追加：Construct validity** | **Part 4.2 限界 11 で対応**（item content review、convergent / discriminant validity check） |
| **追加：Reproducibility** | **D-NEW9 で seed 固定、container 化、make reproduce、aggregated open data** |
