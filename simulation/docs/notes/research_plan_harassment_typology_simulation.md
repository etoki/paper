...
加えて、HH-specific 介入の anchor を選定（Part 6 で詳述）：
- **Lavelock et al. (2014) "Workbook on Humility"**：HH 直接 target、d ≈ 0.3–0.5
- **Stieger et al. (2021) PEACH**：digital coaching、Big Five で d ≈ 0.3–0.5
- **Mazar et al. (2008) Moral reminder**：state-level honesty 抑制、d ≈ 0.2–0.4
...
### 6.2 介入 anchor 候補（あなたの 11 介入リストより）

著者が事前に確認している介入研究（sunblaze.jp の 11 記事）から、Phase 2 で使用する anchor 候補：

#### ★★★ 第一候補：Workbook on Humility（推定 Lavelock et al. 2014）
- **HH を直接 target にした実験的介入**
- 想定効果量：d ≈ 0.3–0.5
- Counterfactual A の anchor に最適
- **要確認**：あなたの記事に書かれている正確な effect size を後で PDF で確認

#### ★★★ 第二候補：Digital Coaching（推定 Stieger et al. 2021 PEACH）
- スマホアプリで scalable
- Big Five 全般で d ≈ 0.3–0.5（PNAS 掲載）
- HH 直接ではないが、Agreeableness / Conscientiousness を経由
- Counterfactual A の代替 anchor

#### ★★★ 第三候補：Moral Reminder（推定 Mazar et al. 2008）
- Honor codes / 10 commandments recall
- 状態的 honesty 抑制、d ≈ 0.2–0.4
- **個人 personality を変えずに行動を抑制** → Counterfactual C の anchor に最適

#### ★★ 第四候補：Volitional Personality Change（推定 Hudson & Fraley 2015）
- 4 ヶ月介入で意図的性格変容
- Big Five で d ≈ 0.2–0.4
- Counterfactual B（targeted、self-selected motivated 層）の anchor

#### ★ その他参考
- Letter of gratitude（Seligman 2005 系列）
- Personality feedback（Hudson & Fraley 系列）
- Behavioral activation
- Online group counseling

### 6.3 介入 anchor 確定の手順

1. あなたから **PDF / 実際の効果量数値** を受け取る
2. 各介入の effect size を整理（介入種類 × target trait × effect size の表）
3. Phase 2 の Counterfactual A/B/C それぞれに最適な anchor を割り当て
4. sensitivity analysis のレンジを決定（lower bound：弱介入、upper bound：強介入）
5. Pre-registration に anchor と effect size のレンジを明記
...
| # | 項目 | 選択肢 | 推奨 | 状態 |
|---|---|---|---|---|
| D8 | 介入 anchor の確定 | あなたの 11 介入記事の正確な effect size 確認 | PDF 受領後決定 | **要 PDF 受領** |
