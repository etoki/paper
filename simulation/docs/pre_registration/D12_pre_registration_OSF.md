...
**Document type**: OSF Standard Pre-Registration v2.0 (internal master version, LOCKED + PUBLIC on OSF)
**Drafted**: 2026-04-29
**Last updated**: 2026-05-21 (v2.0 LOCKED + finalized + Public on OSF)
...
**Status**: 🔒 **LOCKED v2.0 — Finalized & Public on OSF**; Stage 0 code execution **unlocked**
**v2.0 OSF DOI** (LOCKED, Public, current primary): **10.17605/OSF.IO/3Y54U** — https://osf.io/3y54u (registered 2026-04-30, contributor-approved & finalized 2026-05-21)
**v1.1 OSF DOI** (LOCKED, Public, historical, superseded): 10.17605/OSF.IO/45QP9 — https://osf.io/45qp9 (registered 2026-04-30, finalized 2026-05-21)
...
**14-cell（7 type × 2 gender）conditional harassment propensity を population aggregate に scale up した予測値は、MHLW H28（FY2016、pre-law、過去 3 年間 **パワハラ** 被害経験 32.5%）に対して MAPE ≤ 30% の精度で再現される。**

- 主 validation target: **MHLW H28 (FY2016)** 「職場のパワーハラスメントに関する実態調査」（過去 3 年間 power harassment prevalence 32.5%、pre-law）
- 副 validation target: **MHLW R2 (FY2020)** 「職場のハラスメントに関する実態調査」（31.4%、transition）、**MHLW R5 (FY2023, published March 2024)** 「令和5年度 職場のハラスメントに関する実態調査」（19.3%、post-law）
- ★ 重要 scope 注記: 本 study が validate するのは **パワハラ (power harassment) の過去 3 年 prevalence** であり、セクハラ・カスハラ・妊娠出産育児ハラスメント等は別 category。MHLW R5 (FY2023) では同サンプル N=8,000 で セクハラ 6.3%、カスハラ 10.8% も測定されているが、本 study の validation target は **パワハラに限定**。
- 国際 baseline: ILO (2022) Asia-Pacific lifetime 19.2%
- 周辺分布検証 (1): **Pasona Research (2022)** N=28,135 全国就業者 20-69 歳、5 年以内 ハラスメント被害 19.7%（生涯 34.6%）— harassment-specific large-N domestic survey として triangulation
- 周辺分布検証 (2): Tsuno et al. (2015) N=1,546 random sample, 30-day prevalence 6.1%
...
**MHLW H28 (FY2016, pre-law、過去 3 年パワハラ 32.5%) と本 simulation の latent prediction の gap は、MHLW R5 (FY2023, post-law、過去 3 年パワハラ 19.3%) との gap より小さい。** すなわち、pre-law condition は latent-proximal、post-law は environmental gating 強。

**カスハラ category emergence (R4 改正法 2022 年 4 月施行による中小企業含む全事業主対応義務化、MHLW R5 で初めて 10.8% として独立計測)** は、上述の "パワハラ 32.5% → 19.3% 減少" が単なる prevalence 減少ではなく、(a) 法律下で「ハラスメント」と分類される行為の境界自体が変動した（カスハラが新カテゴリとして expressed 化）、(b) 同時に existing パワハラ category の expressed 報告が抑制された、という **複合的な environmental gating の証拠**。Discussion で本研究の latent vs expressed gap が単純な「環境効果」ではなく「category boundary shift × gating strength」の合成であることを honest acknowledge する。
...
| **H2.industry** | **1** | **Secondary confirmatory** | **B4 industry-stratified subgroup MAPE ≤ 50% against MHLW R5 業種別 (16-26.8% range)** ★ 追加 |
| H3 | 1 | Exploratory descriptive | MHLW H28 gap < MHLW R5 gap |
...
Stage 2: Validation triangulation
  ├─ Compare national latent power-harassment prediction to MHLW H28 FY2016 (32.5%, primary) / R2 FY2020 (31.4%) / R5 FY2023 (19.3%)
...
#### 4.2.3 集団 level (MHLW + 民間 large-N、external validation target)

| 変数 | データソース | 役割 |
|---|---|---|
| **過去 3 年間 power harassment 被害経験率** | **MHLW H28 (FY2016, pre-law) 32.5% ★ primary**, MHLW R2 (FY2020, transition) 31.4%, **MHLW R5 (FY2023, post-law, published 2024-03) 19.3%** | National validation target |
| **業種別 power harassment past-3-year 経験率** | **MHLW R5 (FY2023) 業種別: 建設 26.8%, 複合 22.5%, 教育/医療福祉 20.6%, …, 教育 16.9%（業種範囲 16-26.8%、N=8,000 一般 sample）** | **B4 baseline subgroup validation (H2.industry)** |
| **その他ハラスメント category 比較** | MHLW R5 セクハラ 6.3% (女 8.9% / 男 3.9%), カスハラ 10.8%（環境変動 framing 用） | latent vs expressed gap framing (H3) |
| **5 年以内ハラスメント被害経験率（民間）** | **Pasona Research (2022) 「職場のハラスメントについての定量調査」N=28,135、5 年以内 19.7%（生涯 34.6%）、業種別 16.9-22.9%** | 周辺分布検証 (large-N harassment-specific) |
| **30 日 prevalence (random sample)** | Tsuno et al. 2015 N=1,546 (6.1%) | 周辺分布検証 (national-rep) |
| **国際 baseline** | ILO 2022 Asia-Pacific lifetime 19.2% | 比較参照 |
| **f1 anchor (ハラスメント被害者の離職率)** | **Pasona (2022) ハラスメント被害者 5 年以内離職率 10.3% (業種別 6.3-13.3%) ★ primary**; **MHLW R4 雇用動向調査 (FY2022)** 転職入職者前職離職理由「職場の人間関係」男 8.3% / 女 9.4% (upper bound) | f1 sensitivity sweep [0.05, 0.10, 0.15, 0.20] の empirical anchor |
| **f1 macro cross-check** | Pasona (2022) 全国年間ハラスメント離職者推定 86.5 万人（うち暗数 57.3 万人 = 66%）, 全離職者中比率 86.5/717.3 = 12.1% | Stage 2 chain output sanity check (Section 5.4) |
...
**Triangulation against marginal-distribution surveys** (★ pre-registered, descriptive):
...
**Stage 2 chain output sanity check** (★ pre-registered, soft criterion):
- Compute predicted annual harassment-induced turnover via V × f1 chain (Stage 2): Σ_cell (predicted perpetrators × V × f1)
- Compare against **Pasona (2022) 全国年間ハラスメント離職者推定 86.5 万人** (with 暗数 57.3 万人 = 66%)
- Soft criterion: predicted annual turnover within 50-200% of Pasona estimate (i.e., 43-173 万人) — outside this range flagged for failure-mode discussion
...
- Compute MAPE vs MHLW H28 (FY2016) past-3-year power harassment 32.5%
...
- **B4**: 7 type + age + **業種 (estimated)** + 雇用形態 cell-conditional

**Industry estimation specification for B4** (★ pre-registered, ★ Item 5):
- N=354 does **not contain direct industry data**. B4's "業種" is **estimated probabilistically** from age × gender × employment type joint distribution against MHLW 労働力調査 industry × demographic crosstabs.
- Estimation method (★ pre-registered): for each individual i with (age_i, gender_i, employment_i), assign a 16-bucket industry probability vector P(industry_j | age_i, gender_i, employment_i) using MHLW labor force data 2022. The cell prediction for B4 is then weighted by this probability vector.
- 16 industry buckets follow MHLW 日本標準産業分類 main divisions (matching MHLW R5 industry-stratified harassment data structure).
...
**Industry-stratified validation (H2.industry)** (★ pre-registered, ★ Item 5):
...
- **Subgroup MAPE threshold (relaxed)**: 50% (more lenient than overall H1 30%, given industry estimation noise; 業種 is estimated, not observed)
- Decision rule:
  - Industry-MAPE ≤ 50% → CONFIRMS that personality + estimated industry can predict industry-level patterns
  - Industry-MAPE > 50% → REPORT as honest limitation (industry estimation noise dominates personality signal at industry level)
...
|---|---|---|---|
| **H1 (Strict SUCCESS)** ★ v2 | Point MAPE(P̂_FY2016, MHLW H28 FY2016 32.5%) **AND** 95% BCa CI upper bound | both ≤ 30% | Strong confirmatory; "Strict SUCCESS achieved" allowed without qualification |
| **H1 (Standard SUCCESS)** ★ v2 | Point MAPE | ≤ 30% (CI may overlap PARTIAL region) | Weak confirmatory; **Pre-registered ambiguity Tier** note required (Section 5.4) |
| H1 (Partial) | Point MAPE | 30 < x ≤ 60% | Mixed evidence, CI reported in Discussion |
| H1 (Failure) | Point MAPE | > 60% | Publish as failure-mode discovery (Section 7.3) |
...
| **H2.industry** ★ | B4 industry-stratified (16-bucket) MAPE vs MHLW R5 (FY2023) industry-stratified power harassment 16-26.8% | ≤ 50% (relaxed; 業種 estimated) | CONFIRMED if ≤ 50%; honest LIMITATION REPORT if > 50%. Secondary criterion (does not affect H1 SUCCESS / FAILURE). |
| H3 | gap(H28 FY2016) < gap(R5 FY2023) | Direction | Confirmed if MAPE_FY2016 < MAPE_FY2023 |
...
| **f1** (harassment-induced turnover rate) | 0.10 (close to Pasona 2022 empirical 10.3%) | {0.05, 0.10, 0.15, 0.20} (covers Pasona industry range 6.3-13.3% + MHLW R4 「人間関係」離職 8.3-9.4%) | 3 |
...
| MHLW H28 (FY2016) 職場のパワーハラスメント実態調査 | Public | https://www.mhlw.go.jp/ (pre-law, 32.5%, ★ primary validation target H1) |
| MHLW R2 (FY2020) 職場のハラスメント実態調査 報告書 | Public | https://www.mhlw.go.jp/ (transition, 31.4%) |
| **MHLW R5 (FY2023, published March 2024) 令和5年度 厚生労働省委託事業 職場のハラスメントに関する実態調査報告書** (PwC コンサルティング合同会社, 385 pages) | Public | `simulation/prior_research/_text/令和５年度 厚生労働省委託事業 職場のハラスメントに関する実態調査報告書.pdf` (post-law, 19.3%, full report; industry-stratified data 16-26.8% used in B4 H2.industry validation) |
| **MHLW R4 (FY2022) 雇用動向調査結果の概況** | Public | `simulation/prior_research/_text/厚生労働省_令和４年雇用動向調査結果の概況.pdf` (FY2022 turnover by reason "interpersonal" 8.3-9.4%; f1 secondary anchor) |
| MHLW 労働安全衛生調査 | Public | https://www.mhlw.go.jp/ |
| MHLW 労働力調査 | Public | https://www.stat.go.jp/data/roudou/ (industry × demographic crosstabs for B4 industry estimation, Section 5.5) |
| ILO 2022 Global survey | Public | https://www.ilo.org/ |
| Tsuno et al. 2015 N=1,546 | Published | Tsuno et al. 2015 PLOS ONE (national-rep 30-day prevalence 6.1%) |
| Tsuno & Tabuchi 2022 | Published | Tsuno & Tabuchi 2022 (PR=3.20 for f2 anchor) |
| **Pasona Research (2022) 職場のハラスメントについての定量調査** (株式会社パーソル総合研究所, シンクタンク本部, 152 pages) | Industry survey | `simulation/prior_research/_text/パーソル_職場のハラスメントについての定量調査.pdf` (★ N=28,135 large-N harassment-specific 5-year prevalence 19.7%, 業種別 16.9-22.9%; primary f1 anchor 10.3% [業種別 6.3-13.3%]; macro turnover estimate 86.5万人/年 with 暗数 66%) |
...
- **MHLW (H28, FY2016)** 平成28年度 職場のパワーハラスメントに関する実態調査. [→ ★ H1 primary validation target, 32.5% pre-law power harassment past-3-year]
- **MHLW (R2, FY2020, published 2021)** 令和2年度 職場のハラスメントに関する実態調査 報告書. [→ secondary validation 31.4% transition]
- **MHLW (R5, FY2023, published March 2024)** 令和5年度 厚生労働省委託事業 職場のハラスメントに関する実態調査報告書 (PwC コンサルティング合同会社, 385 pages). [→ ★ secondary validation 19.3% post-law; ★ industry-stratified data 16-26.8% used in H2.industry secondary criterion (Section 5.5, 6.1); カスハラ 10.8% as new environmental category (Section 1.4 H3)]
- **MHLW (R4, FY2022, published August 2023)** 令和4年雇用動向調査結果の概況. [→ ★ f1 secondary anchor: turnover by reason "interpersonal" 男 8.3% / 女 9.4% as upper bound]
- **Pasona Research (2022)** 職場のハラスメントについての定量調査 (株式会社パーソル総合研究所, シンクタンク本部, N=28,135 全国就業者 20-69 歳, 152 pages). [→ ★ Pre-reg validation triangulation (Section 5.4): 5-year harassment prevalence 19.7%, industry-stratified 16.9-22.9%; ★ f1 PRIMARY empirical anchor: harassment-victim turnover rate 10.3% (industry range 6.3-13.3%) (Section 4.2.3, 6.4); ★ Stage 2 chain output sanity check (Section 5.4): macro turnover 86.5万/年 with 66% 暗数化]
- **Tsuno et al. (2010)** Japanese NAQ-R. [→ measurement]
- **Tsuno et al. (2015)** Socioeconomic determinants Japan national rep N=1,546 (PLOS ONE). [→ Section 1.5, 4.2; 30-day prevalence 6.1% as reference]
...
| **v2.0 PUBLIC FINALIZED** | 2026-05-21 | 🌐 Both v1.1 and v2.0 registrations contributor-approved (sole-author) and finalized to public state on OSF. DOIs now resolve via `https://doi.org/10.17605/OSF.IO/3Y54U` (v2.0 primary) and `https://doi.org/10.17605/OSF.IO/45QP9` (v1.1 historical). Public visibility satisfies Sections 7.3 (negative-result publication commitment), 8.0 row 4 (Munafò Theme 4 post-pub review), 9.6 (10-year ethical monitoring), and 6.5 Level 3 ("with public diff" requirement). Methods Clarifications Log v1.0 (Section 6.5 Level 1 deviation) is uploaded to OSF Associated Project Files (osf.io/3hxz6/v2.0/) and discoverable via Project Wiki pointer. The preregistration is now fully functional as a citable, verifiable, locked record. |

### 14.2 Pre-registration submission checklist

#### 14.2.1 OSF pre-reg 登録（独立した track）
...
- [x] **OSF Standard Pre-Registration template**: Sections 1–6 を OSF Web フォームに転記済（Part B paste sheet 使用）
- [x] **PDF supplementary**: JP/EN markdown + JP/EN PDF を OSF に attach 済
- [x] **DOI 取得**: 10.17605/OSF.IO/45QP9 取得済、本ドキュメント Header に記録済
- [x] **GitHub mirror**: `simulation/docs/pre_registration/` を public commit (commit `c5c591e` 以降で push 済)
...
- [ ] **Independent methodologist contact**: Section 8.1 review 依頼確定（mode B: anonymous, math bio）

#### 14.2.2 Registered Report 投稿（pre-reg lock 後の別 track）

- [ ] RSOS Registered Report **Author Guidelines** 確認（最新版、scope / format requirement）
- [ ] **Stage 1 manuscript** 準備（Introduction + Methods + 本 pre-reg を統合した英文原稿、Stage 1 用 word limit 内）
  - Introduction: 研究計画 v6/v7 Part 10.3 の 5 段落構成を base に英訳
  - Methods: 本 pre-reg Section 4–5 を base に英訳、computational reproducibility (D-NEW9) を含む
- [ ] **Cover letter** 作成（既存 OSF pre-reg DOI 参照、IPA 取得を依頼）
- [ ] **Submission portal**（RSOS / Editorial Manager 等）への投稿
- [ ] **IPA review 対応**（typically 6–8 週、reviewer comments への revision）
- [ ] **IPA acquired** → Stage 0 コード実装着手（Section 14.3）
- [ ] **Stage 2 manuscript**（Results + Discussion）作成、IPA を base に Stage 2 投稿
- [ ] **Final acceptance** → publication

#### 14.2.3 Fallback path（IPA refused 時、Section 7.2 fallback table 参照）

- [ ] Frontiers in Psychology RR specialty に再投稿、または
- [ ] Standard track に切替（Stage 0–8 完了後に Sci Reports / PLOS ONE 等に投稿）
- [ ] OSF pre-reg DOI を必ず引用、deviation log を transparent に記載
...
- [x] **Methods Clarifications Log v1.0** lock 済（2026-05-21、`D12_v2.0_methods_clarifications.md`、Section 6.5 Level 1 deviation log として OSF v2.0 project に supplementary upload 予定。Major×4 + Minor×7 + Nitpicks×5 を反映、5th tier proposal は SKIP）
