...
- **Sole author**: Tokiwa (ORCID: TBD — fill in before OSF submission)
- Co-authors: なし（sole-authored study）
- **Independent methodologist consultation**: pre-reg lock 後・Stage 1 validation 着手前に外部方法論者に Section 5 (Analysis Plan) のみ review 依頼予定（authorship なし、Acknowledgments 記載のみ、Munafò et al. 2017 Box 1 CHDI Foundation 例の軽量化適用）
...
- N=354 / N=13,668 データの収集には外部資金（要記載）を受領
- Pre-registration の OSF 登録は無料、author cost なし

### 12.2 Funding

- (要 fill in: data collection grant 番号、PI、affiliation)
- 本研究 simulation phase は無外部資金（既存データ再利用）
...
- [ ] **OSF account & project**: 既存 OSF account に新規 project 作成
...
- [ ] **Funding & affiliation**: Section 12.2 fill-in
