...
- **Counterfactual B** (cluster reassignment): do(cluster ∈ {0, 4, 6}) paired with a +0.40 SD shift in Honesty-Humility so the post-intervention HEXACO scores are consistent with the target-cluster centroids. Individuals are computationally reassigned to one of three low-harassment-prevalence typologies. This intervention represents an extreme (and arguably implausible) personality-shift scenario, included as an upper bound on what any person-level intervention could achieve.
...
4. **Policy-relevant**: It provides counterfactual evidence that the 2020 Power Harassment Prevention Law's structural mechanism is approximately 5.7× more effective at the population level than the personality-training interventions to which Japanese corporate HR programs allocate the majority of harassment-prevention budget.
