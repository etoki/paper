...
#### Step 1a: 残 5 本の PDF 取得待ち（ブロッカー）
以下 5 本の PDF 取得完了まで simulation 重みは暫定値のまま保留:
- A-01 Abe (2020) — PDF request pending via ResearchGate
- A-05 Audet (2023) — PDF request pending
- A-11 Cheng (2023) — PDF request pending
- A-12 Cohen & Baruth (2017) — PDF request pending
- A-27 Wu & Yu (2024) — PDF request pending

**理由**: 質的 synthesis（`literature_review.md`）では方向性（C 正 / O 正 / N 負 / A 弱正 / E ≈0）は 22 本で既に収束しているが、pooled r 算出には残 5 本の効果量が必要。Part 1 の合成データ生成前に全 27 本の数値を確定させる方針。

#### Step 1b: PDF 到着後の作業
1. `metaanalysis/data_extraction.csv` の `r_*_outcome` 列を 27 本分埋める
2. `metafor` で random-effects meta-analysis → pooled r と 95% CI
3. Pooled r を `simulation/HANDOFF.md` §4「Big Five 予測重み」に反映
